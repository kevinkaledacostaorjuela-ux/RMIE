<?php
/**
 * API Endpoint - Guardar planificación semanal (versión de prueba)
 */
    ...
    try {
        ...
        foreach ($planificacion as $dia => $clientes) {
            ...
            if (is_array($clientes) && !empty($clientes)) {
                foreach ($clientes as $index => $cliente_obj) {
                    // Manejar tanto objetos como IDs directos
                    if (is_array($cliente_obj) && isset($cliente_obj['id_clientes'])) {
                        $cliente_id = (int)$cliente_obj['id_clientes'];
                    } else {
                        $cliente_id = (int)$cliente_obj;
                    }
