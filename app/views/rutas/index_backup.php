...
<head>
    ...
    <style>
        ...
        /* Responsividad para planificación semanal */
        @media (max-width: 768px) {
            .dias-semana-grid {
                grid-template-columns: 1fr;
                gap: 15px;
        ...
        <div class="mb-4">
            <div class="action-buttons-container">
                <div class="primary-actions">
                    <a href="/RMIE/rutas.php?accion=create" class="btn btn-modern btn-success-modern btn-lg">
            ...
            <div id="rutasCardsContainer" class="routes-grid" style="display: none;">
                ...
                <?php else: ?>
                    <div class="routes-empty-state">
                        ...
                        <a href="/RMIE/rutas.php?accion=create" class="btn btn-modern btn-success-modern">
...
window.addEventListener('unhandledrejection', function(e) { console.log('Promise Error:', e.reason); e.preventDefault(); });
        ...
        function crearRutaRapida() {
            window.location.href = '/RMIE/rutas.php?accion=create';
        }
        ...
        function completarRutaModal(idRuta, nombreRuta) {
            ...
            window.ejecutarCompletarRutaSimple = function(id) {
                ...
                window.location.href = `/RMIE/rutas.php?accion=${accion}&id=${id}`;
            };
        }
        ...
        function limpiarCompletadasAjax() {
            ...
            if (confirm('¿Estás seguro de que quieres eliminar TODAS las rutas completadas?\n\nEsta acción no se puede deshacer y eliminará permanentemente todas las rutas con estado "completada".')) {
                ...
                fetch('/RMIE/rutas.php?accion=clean_completed', {
                    method: 'GET'
                })
                .then(response => {
        ...
        async function cargarPlanificacion() {
            ...
            try {
                const promises = diasSemana.map(dia => 
                    fetch(`/RMIE/app/api/test_route_info.php?dia=${dia.toLowerCase()}`)
                        .then(response => response.json())
                        .then(data => ({ dia, data }))
                        .catch(error => ({ dia, data: { success: false, error: error.message }}))
                );
                ...
                // Procesar resultados
                planificacionData = {};
                resultados.forEach(({ dia, data }) => {
                    if (data.success && data.data && data.data.clientes_sugeridos) {
                        planificacionData[dia] = data.data.clientes_sugeridos.map(c => c.id);
                    } else {
                        planificacionData[dia] = [];
                    }
                });

                // Obtener lista de todos los clientes disponibles
                const clientesResponse = await fetch('/RMIE/app/api/test_get_available_clients.php');
                const clientesData = await clientesResponse.json();
                
                if (clientesData.success) {
                    window.availableClients = clientesData.clients;
                } else {
                ...
                mostrarNotificacionPlanificacion('Error al cargar la planificación', 'error');
            } finally {
                mostrarCargandoPlanificacion(false);
            }
        }

        // Renderizar los días de la semana
        function renderizarDiasSemana() {
            const container = document.getElementById('diasSemanaGrid');
            const diaHoy = new Date().getDay();
            const diasHoyNombre = ['Domingo', 'Lunes', 'Martes', 'Miércoles', 'Jueves', 'Viernes', 'Sábado'];
            const hoyNombre = diasHoyNombre[diaHoy];

            // Eliminar indicador de carga inicial
            const loadingInicial = container.querySelector('.loading-inicial');
            if (loadingInicial) {
                loadingInicial.style.animation = 'fadeOut 0.3s ease-out';
                setTimeout(() => {
                    container.innerHTML = '';
                    renderizarCards();
                }, 300);
            ...
            diasSemana.forEach(dia => {
                const clientesDelDia = planificacionData[dia] || [];
                const esHoy = dia === hoyNombre;

                const diaCard = document.createElement('div');
                diaCard.className = `dia-card ${esHoy ? 'dia-hoy' : ''}`;
                diaCard.innerHTML = `
                    <div class="dia-header">
                        ...
                        <div class="dia-badge">${clientesDelDia.length} cliente${clientesDelDia.length !== 1 ? 's' : ''}</div>
                    </div>
                    <div class="dia-clientes" id="clientes-${dia}">
                        ${renderizarClientesDia(clientesDelDia, dia)}
                    </div>
                    <button class="btn-modern btn-success-modern mt-2" onclick="agregarClienteDia('${dia}')" style="width: 100%; font-size: 0.85rem;">
        ...
        // Renderizar clientes de un día específico
        function renderizarClientesDia(clientesIds, dia) {
            if (!clientesIds || clientesIds.length === 0) {
                return `
                    <div class="dia-empty">
                        <i class="fas fa-calendar-alt" style="font-size: 1.5rem; margin-bottom: 8px; opacity: 0.5;"></i>
                        <div>No hay clientes asignados</div>
                        <small style="opacity: 0.7;">Haz clic en "Agregar Cliente" para comenzar</small>
                    </div>
                `;
            }

            return clientesIds.map((clienteId, index) => {
                const cliente = window.availableClients?.find(c => c.id == clienteId);
                const nombre = cliente ? cliente.nombre : `Cliente #${clienteId}`;
                const descripcion = cliente?.descripcion || '';
                
                return `
                    <div class="cliente-item" data-cliente-id="${clienteId}" style="animation: slideInLeft 0.3s ease-out ${index * 0.1}s both;">
                        <div class="cliente-info">
                            <div class="cliente-nombre">
                                <i class="fas fa-user-circle"></i>
                                <span>${nombre}</span>
                                ${descripcion ? `<small style="opacity: 0.8; display: block; margin-top: 2px;">${descripcion}</small>` : ''}
                            </div>
                        </div>
                        <button class="cliente-remove" onclick="removerClienteDia('${dia}', ${clienteId})" title="Remover cliente">
        ...
        // Agregar cliente a un día
        function agregarClienteDia(dia) {
            if (!window.availableClients || window.availableClients.length === 0) {
                mostrarNotificacionPlanificacion('No hay clientes disponibles', 'warning');
                return;
            }

            // Crear modal de selección de cliente
            ...
            const clientesYaAsignados = planificacionData[dia] || [];
            const clientesDisponibles = window.availableClients.filter(c => 
                !clientesYaAsignados.includes(parseInt(c.id))
            );
                ...
                ">
                    <div style="text-align: center; margin-bottom: 25px;">
                        <div style="
                            ...
                            margin-bottom: 15px; box-shadow: 0 8px 25px rgba(0,0,0,0.2);
                        ">
                            <i class="fas fa-user-plus" style="color: white; font-size: 1.5rem;"></i>
                        </div>
                        <h3 style="color: #333; margin: 0; font-weight: 600;">Agregar Cliente a ${dia}</h3>
                        <p style="color: #666; margin: 8px 0 0 0; font-size: 0.9rem;">
                            Selecciona un cliente para asignar a este día
                        </p>
                    </div>
                    ...
                    <div style="margin-bottom: 20px;">
                        <div style="position: relative;">
                            <input type="text" id="buscarClienteInput" placeholder="Buscar cliente por nombre..." style="
                                width: 100%; padding: 12px 40px 12px 15px; border: 2px solid #e0e0e0; 
                                border-radius: 12px; font-size: 1rem; outline: none;
                                transition: all 0.3s ease;
                            " onkeyup="filtrarClientes(this.value)" onfocus="
                    ...
                    <div id="infoClienteSeleccionado" style="
                        display: none; background: linear-gradient(135deg, #667eea20, #764ba220); 
                        border-radius: 12px; padding: 15px; margin-bottom: 20px; border: 1px solid #667eea40;
                    ">
                        <div style="display: flex; align-items: center; gap: 12px;">
                            <i class="fas fa-info-circle" style="color: #667eea; font-size: 1.2rem;"></i>
                            <div>
                                <div id="clienteSeleccionadoNombre" style="font-weight: 600; color: #333;"></div>
                                <div id="clienteSeleccionadoLocal" style="color: #666; font-size: 0.9rem; margin-top: 2px;"></div>
                            </div>
                        </div>
                    </div>

                    ${clientesDisponibles.length > 0 ? `
                        <div id="listaClientes" style="margin-bottom: 25px; max-height: 300px; overflow-y: auto;">
                            ${clientesDisponibles.map(cliente => `
                                <div class="cliente-modal-item" data-cliente-nombre="${cliente.nombre.toLowerCase()}" onclick="seleccionarClienteConLocal(${cliente.id}, '${dia}', '${cliente.nombre}', this)" style="
                                ...
                                ">
                                    <div style="display: flex; align-items: center; gap: 12px;">
                                        <div style="
                                            width: 40px; height: 40px; background: linear-gradient(45deg, #4facfe, #00f2fe);
                                        ...
                                        <div style="flex: 1;">
                                            ...
                                            <div style="color: #999; font-size: 0.75rem; margin-top: 4px;">
                                                ID: ${cliente.id} ${cliente.telefono ? `• Tel: ${cliente.telefono}` : ''}
                                            </div>
                                            <div class="cliente-local-info" style="
                                                color: #667eea; font-size: 0.8rem; margin-top: 6px; 
                                                background: rgba(102, 126, 234, 0.1); padding: 4px 8px; 
                                                border-radius: 6px; display: inline-block;
                                            ">
                                                <i class="fas fa-store"></i> <span class="local-nombre">Cargando local...</span>
                                            </div>
                                        </div>
                                        <div style="
                                            width: 24px; height: 24px; border: 2px solid #667eea; border-radius: 50%;
                                            display: flex; align-items: center; justify-content: center; opacity: 0.3;
                                        ">
                                            <i class="fas fa-plus" style="font-size: 0.7rem; color: #667eea;"></i>
                                        </div>
                                    </div>
                                </div>
                            `).join('')}
                    ...
                    ` : `
                        <div style="text-align: center; padding: 40px 20px; color: #666;">
                            ...
                            <p style="margin: 0; font-size: 1.1rem;">No hay clientes disponibles</p>
                            <p style="margin: 5px 0 0 0; font-size: 0.9rem; opacity: 0.8;">
                                Todos los clientes ya están asignados a este día
                            </p>
                        </div>
                    `}
                    
                    <div style="text-align: center; border-top: 1px solid #eee; padding-top: 20px;">
                        <button onclick="cerrarModalCliente()" style="
                            background: linear-gradient(45deg, #6c757d, #5a6268); color: white; border: none; 
                            border-radius: 12px; padding: 12px 24px; cursor: pointer; font-weight: 500;
            ...
            setTimeout(() => {
                cargarLocalesClientes();
            }, 100);
        }

        // Seleccionar cliente del modal
        function seleccionarCliente(clienteId, dia, element) {
            if (!planificacionData[dia]) {
                planificacionData[dia] = [];
            }
            
            planificacionData[dia].push(clienteId);
            
            // Actualizar vista del día
            const container = document.getElementById(`clientes-${dia}`);
            container.innerHTML = renderizarClientesDia(planificacionData[dia], dia);
            
            // Actualizar contador
            const badge = container.parentElement.querySelector('.dia-badge:last-child');
            const count = planificacionData[dia].length;
            badge.textContent = `${count} cliente${count !== 1 ? 's' : ''}`;
            
            cerrarModalCliente();
            mostrarNotificacionPlanificacion(`Cliente agregado a ${dia}`, 'success');
        }

        // Filtrar clientes por nombre
        function filtrarClientes(busqueda) {
            const clientesItems = document.querySelectorAll('.cliente-modal-item');
            const busquedaLower = busqueda.toLowerCase();
            
            clientesItems.forEach(item => {
                const nombreCliente = item.getAttribute('data-cliente-nombre');
        ...
        // Seleccionar cliente y mostrar su local asignado
        async function seleccionarClienteConLocal(clienteId, dia, nombreCliente, element) {
            // Mostrar información del cliente seleccionado
            const infoPanel = document.getElementById('infoClienteSeleccionado');
            const nombreSpan = document.getElementById('clienteSeleccionadoNombre');
            const localSpan = document.getElementById('clienteSeleccionadoLocal');
            
            nombreSpan.textContent = nombreCliente;
            localSpan.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Obteniendo información del local...';
            infoPanel.style.display = 'block';
            
            try {
                // Obtener el local asignado al cliente
                const response = await fetch(`/RMIE/app/api/get_cliente_local.php?cliente_id=${clienteId}`);
                const result = await response.json();
                
                if (result.success && result.local) {
                    localSpan.innerHTML = `<i class="fas fa-store"></i> <strong>Local:</strong> ${result.local.nombre} - ${result.local.direccion || 'Sin dirección especificada'}`;
                } else {
                    localSpan.innerHTML = '<i class="fas fa-exclamation-triangle"></i> <strong>Local:</strong> No asignado o no encontrado';
                }
                
                // Esperar un momento para que el usuario vea la información
                setTimeout(() => {
                    // Preparar información detallada para confirmación
                    let infoLocal = 'No asignado';
                    if (result.local) {
                        infoLocal = `${result.local.nombre}\\n📍 ${result.local.direccion}`;
                        if (result.local.telefono && result.local.telefono !== 'Sin teléfono') {
                            infoLocal += `\\n📞 ${result.local.telefono}`;
                        }
                    }
                    ...
                    if (confirmar) {
                        ...
                        // Ocultar panel si cancela
                        infoPanel.style.display = 'none';
                    }
                }, 1500);
                
            } catch (error) {
                console.error('Error obteniendo local del cliente:', error);
                localSpan.innerHTML = '<i class="fas fa-exclamation-circle"></i> <strong>Local:</strong> Error al obtener información';
        ...
        // Cargar locales para cada cliente cuando se abre el modal
        async function cargarLocalesClientes() {
            const localInfos = document.querySelectorAll('.cliente-local-info .local-nombre');
            
            for (const localInfo of localInfos) {
                const clienteItem = localInfo.closest('.cliente-modal-item');
                const onclickAttr = clienteItem.getAttribute('onclick');
                const clienteIdMatch = onclickAttr.match(/seleccionarClienteConLocal\((\d+),/);
                ...
                if (clienteIdMatch) {
                    ...
                    try {
                        ...
                        if (result.success && result.local) {
                            localInfo.textContent = result.local.nombre;
                        } else {
                            localInfo.textContent = 'Sin local asignado';
                            localInfo.parentElement.style.background = 'rgba(255, 152, 0, 0.1)';
                            localInfo.parentElement.style.color = '#ff9800';
                        }
                    } catch (error) {
                        localInfo.textContent = 'Error al cargar';
                        localInfo.parentElement.style.background = 'rgba(244, 67, 54, 0.1)';
                        localInfo.parentElement.style.color = '#f44336';
                    }
                }
            }
        }
        ...
        // Remover cliente de un día
        function removerClienteDia(dia, clienteId) {
            if (!planificacionData[dia]) return;
            
            planificacionData[dia] = planificacionData[dia].filter(id => id != clienteId);
            ...
            mostrarNotificacionPlanificacion('Cliente removido', 'info');
        }

        // Guardar planificación en el servidor
        async function guardarPlanificacion() {
            // Verificar que hay datos para guardar
            const totalClientes = Object.values(planificacionData).reduce((total, clientes) => total + (clientes?.length || 0), 0);
            
            if (totalClientes === 0) {
                mostrarNotificacionPlanificacion('No hay clientes asignados para guardar', 'warning');
                return;
            }

            mostrarCargandoPlanificacion(true);
            cambiarEstadoBotonGuardar('guardando');
            
            try {
                // 1. Guardar la planificación
                mostrarNotificacionPlanificacion('Guardando planificación semanal...', 'info');
                ...
                if (result.success) {
                    mostrarNotificacionPlanificacion(`✓ Planificación guardada: ${result.total_asignaciones} asignaciones`, 'success');
                    
                    // 2. Crear las rutas automáticamente
                    mostrarNotificacionPlanificacion('Creando rutas automáticamente...', 'info');
                    const rutasResult = await crearRutasDesdeplanificacion();
                    
                    if (rutasResult && rutasResult.success) {
                        // 3. Actualizar la tabla de rutas
                        mostrarNotificacionPlanificacion('Actualizando tabla de rutas...', 'info');
                        await actualizarTablaRutas();
                        
                        cambiarEstadoBotonGuardar('completado');
                    } else {
                        cambiarEstadoBotonGuardar('error');
                    }
                    
                } else {
                    mostrarNotificacionPlanificacion(result.message || 'Error al guardar', 'error');
                    cambiarEstadoBotonGuardar('error');
                }
            } catch (error) {
                console.error('Error guardando planificación:', error);
                mostrarNotificacionPlanificacion('Error al guardar la planificación', 'error');
                cambiarEstadoBotonGuardar('error');
            } finally {
                mostrarCargandoPlanificacion(false);
                ...
                setTimeout(() => {
                    cambiarEstadoBotonGuardar('normal');
                }, 2000);
            }
        }
        ...
        function cambiarEstadoBotonGuardar(estado) {
            const botones = document.querySelectorAll('.planificacion-actions button');
            const botonGuardar = Array.from(botones).find(btn => btn.textContent.includes('Guardar'));
            ...
            switch (estado) {
                case 'guardando':
                    ...
                    botonGuardar.style.background = 'linear-gradient(45deg, #ff9a9e, #fecfef)';
                    break;
                    
                case 'completado':
                    botonGuardar.innerHTML = '<i class="fas fa-check"></i> ¡Completado!';
                    botonGuardar.style.background = 'linear-gradient(45deg, #4facfe, #00f2fe)';
                    break;
                    
                case 'error':
                    botonGuardar.innerHTML = '<i class="fas fa-exclamation-triangle"></i> Error';
                    botonGuardar.style.background = 'linear-gradient(45deg, #ff6b6b, #ee5a52)';
                    break;
                    
                default: // normal
                    botonGuardar.innerHTML = '<i class="fas fa-save"></i> Guardar Planificación';
                    botonGuardar.disabled = false;
                    botonGuardar.style.background = 'linear-gradient(45deg, #667eea, #764ba2)';
                    break;
            }
        }

        // Crear rutas automáticamente desde la planificación
        async function crearRutasDesdeplanificacion() {
            try {
                ...
                if (result.success) {
                    mostrarNotificacionPlanificacion(`${result.rutas_creadas} rutas creadas exitosamente`, 'success');
                } else {
                    mostrarNotificacionPlanificacion('Error creando rutas: ' + result.message, 'warning');
                }
                
                return result;
            } catch (error) {
                console.error('Error creando rutas:', error);
                mostrarNotificacionPlanificacion('Error al crear rutas automáticamente', 'warning');
            }
        }

        // Actualizar la tabla de rutas
        async function actualizarTablaRutas() {
            try {
                // Obtener las rutas más recientes
                const response = await fetch('/RMIE/app/api/get_rutas_recientes.php');
                const result = await response.json();
                
                if (result.success && result.rutas.length > 0) {
                    // Actualizar contenido de la tabla
                    actualizarContenidoTabla(result.rutas);
                    mostrarNotificacionPlanificacion(`${result.rutas.length} rutas agregadas a la tabla`, 'info');
                } else {
                    // Si no hay rutas o hay error, recargar la página
                    setTimeout(() => {
                        window.location.reload();
                    }, 1000);
                }
        ...
        // Actualizar contenido de la tabla con las nuevas rutas
        function actualizarContenidoTabla(rutasNuevas) {
            // Buscar el contenedor de la tabla
            const tablaContainer = document.querySelector('.table-container .table-responsive');
            const tabla = tablaContainer?.querySelector('table tbody');
            const emptyMessage = document.querySelector('.empty-state');
            
            if (emptyMessage) {
                // Si hay mensaje de "No hay rutas", eliminarlo
                emptyMessage.remove();
            }
            
            if (!tabla) {
                // Si no existe la tabla, crearla
                crearTablaNueva(tablaContainer, rutasNuevas);
            } else {
                // Si existe, agregar las nuevas filas
                agregarFilasATabla(tabla, rutasNuevas);
            }
        }

        // Crear tabla nueva si no existe
        function crearTablaNueva(container, rutas) {
            const tablaHTML = `
                <table class="table table-modern">
                    ...
                    <tbody>
                        ${rutas.map(ruta => crearFilaRuta(ruta)).join('')}
                    </tbody>
                </table>
            `;
        ...
        function agregarFilasATabla(tbody, rutas) {
            rutas.forEach(ruta => {
                const fila = document.createElement('tr');
                fila.innerHTML = crearFilaRuta(ruta, true);
                fila.style.animation = 'slideInUp 0.5s ease-out';
                tbody.appendChild(fila);
            });
        }

        // Crear HTML para una fila de ruta
        function crearFilaRuta(ruta, esNueva = false) {
            const estiloNuevo = esNueva ? 'background: rgba(76, 175, 80, 0.1); border-left: 4px solid #4CAF50;' : '';
            
            return `
                <td style="${estiloNuevo}"><strong>#${ruta.id_ruta}</strong></td>
                <td style="${estiloNuevo}">Ruta Planificada</td>
                ...
                <td style="${estiloNuevo}">${new Date().toLocaleDateString()}</td>
                <td style="${estiloNuevo}">
                    <button class="btn-modern btn-primary-modern" onclick="editarRuta(${ruta.id_ruta})" title="Editar">
                        <i class="fas fa-edit"></i>
                    </button>
                    <button class="btn-modern btn-danger-modern" onclick="eliminarRuta(${ruta.id_ruta})" title="Eliminar">
                        <i class="fas fa-trash"></i>
                    </button>
                </td>
            `;
        ...
        function resetearPlanificacion() {
            if (confirm('¿Estás seguro de que quieres limpiar toda la planificación semanal?')) {
                ...
                renderizarDiasSemana();
                mostrarNotificacionPlanificacion('Planificación restablecida', 'info');
            }
        }

        // Mostrar indicador de carga
        function mostrarCargandoPlanificacion(mostrar) {
        ...
        function mostrarNotificacionPlanificacion(mensaje, tipo = 'info') {
            const colores = {
                success: '#4facfe',
                error: '#ff6b6b', 
                warning: '#ff9a9e',
            ...
            notif.style.cssText = `
                ...
                background: ${colores[tipo]}; color: white; padding: 15px 20px;
